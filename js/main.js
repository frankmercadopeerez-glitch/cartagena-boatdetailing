...
} /* ===== i18n: language switcher + browser-language auto-detect ===== */
var I18N_PAIRS = {
...
var I18N_PAIRS_REV = {};
Object.keys(I18N_PAIRS).forEach(function (k) {
  I18N_PAIRS_REV[I18N_PAIRS[k]] = k;
});
function i18nCurrentFile() {
  var f = (window.location.pathname || "/").toLowerCase().split("/").pop();
  return f ? f : "index.html";
}
function i18nInfo(prefix) {
  prefix = prefix || "";
  var f = i18nCurrentFile();
  var docLang = (document.documentElement.lang || "es")
    .slice(0, 2)
    .toLowerCase();
  var isEN = docLang === "en";
  var esUrl, enUrl;
  if (isEN) {
    enUrl = prefix + f;
    var es = I18N_PAIRS_REV[f];
    esUrl = es ? prefix + es : null;
  } else {
    esUrl = f === "index.html" ? prefix || "/" : prefix + f;
    var en = I18N_PAIRS[f];
    enUrl = en ? prefix + en : null;
  }
  return { current: isEN ? "en" : "es", esUrl: esUrl, enUrl: enUrl };
}
function buildLangSwitcher(prefix) {
  var i = i18nInfo(prefix);
  var esA = i.current === "es",
    enA = i.current === "en";
  var es =
    '<a href="' +
    (i.esUrl || prefix || "/") +
...
function buildLangSwitcherMobile(prefix) {
  return buildLangSwitcher(prefix).replace(
    'class="lang-switch"',
...
function i18nSetPref(v) {
  try {
    localStorage.setItem("cbd_lang_pref", v);
  } catch (e) {}
}
function i18nBanner(html) {
  if (document.getElementById("cbd-lang-banner")) return;
  var b = document.createElement("div");
  b.id = "cbd-lang-banner";
...
function i18nAutoDetect() {
  var i = i18nInfo(getLayoutContext().prefix);
  if (i.current === "en") return;
  if (i18nGetPref()) return;
  var langs =
...
function i18nInjectStyles() {
  if (document.getElementById("cbd-i18n-styles")) return;
  var s = document.createElement("style");
  s.id = "cbd-i18n-styles";
  s.textContent = `
...
function initLangSwitcher() {
  ...
  document.addEventListener("click", function (e) {
    var th = e.target.closest("[data-translate-help]");
    if (th) {
      e.preventDefault();
      i18nSetPref("en");
      i18nTranslateHelp();
    ...
    var dm = e.target.closest("[data-lang-dismiss]");
    if (dm) {
      e.preventDefault();
      i18nSetPref("dismissed");
      var b = document.getElementById("cbd-lang-banner");
      if (b) {
    ...
    var go = e.target.closest("[data-lang-go]");
    if (go) {
      i18nSetPref("en");
      return;
    }
    var opt = e.target.closest(".lang-switch a.lang-opt");
    if (opt) {
      i18nSetPref(opt.getAttribute("lang") === "en" ? "en" : "es");
    }
  });
}
...
function normalizeFloatingWhatsApp() {
  ...
  ((n.href = a),
    ...
    (n.innerHTML = `<img src="${t}images/whatsapp.webp" alt="WhatsApp" class="whatsapp-float__img" width="58" height="58" />`),
    e.replaceWith(n));
}
...
function initDropdownMenu() {
  ...
  (document.querySelectorAll("[data-dropdown-toggle]").forEach((t) => {
    ...
    document.addEventListener("click", function (t) {
      t.target.closest(".nav-dropdown") || closeAll();
    }),
    document.addEventListener("keydown", function (t) {
      "Escape" === t.key && closeAll();
    }));
}
