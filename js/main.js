  ...
  <div class="container mx-auto px-6 py-10 md:py-14">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
      ...
      <div>
        ...
        <ul class="space-y-2 text-xs font-light">
          <li><a href="${e}hull-cleaning-en.html#zonas-cobertura" class="text-slate-300 hover:text-gold-400 transition-colors flex items-center gap-2"><span class="text-gold-400 opacity-50">›</span> Bocagrande</a></li>
          <li><a href="${e}hull-cleaning-en.html#zonas-cobertura" class="text-slate-300 hover:text-gold-400 transition-colors flex items-center gap-2"><span class="text-gold-400 opacity-50">›</span> Barú</a></li>
          <li><a href="${e}hull-cleaning-en.html#zonas-cobertura" class="text-slate-300 hover:text-gold-400 transition-colors flex items-center gap-2"><span class="text-gold-400 opacity-50">›</span> Rosario Islands</a></li>
        </ul>
      </div>
...
function simplifyServicesNavigation() {
  const { prefix } = getLayoutContext();
  const english = isEnglishDocument();
  const items = english
    ? [
  ...
  const allServicesLabel = english ? "View all services" : "Ver todos los servicios";
  const allServicesHref = english ? "services-en.html" : "services.html";
  const primaryLabel = english ? "Most requested" : "Más solicitado";

  document.querySelectorAll(".nav-dropdown-content").forEach((menu) => {
    menu.innerHTML = items
      .map(
        ([href, label], index) =>
          `<a href="${prefix}${href}" class="nav-dropdown-link${index === 0 ? " nav-dropdown-link--primary" : ""}">${index === 0 ? `<span class="nav-service-badge">★ ${primaryLabel}</span>` : ""}<span>${label}</span></a>`,
      )
      .join("");
    menu.insertAdjacentHTML(
      "beforeend",
      `<a href="${prefix}${allServicesHref}" class="nav-dropdown-link nav-dropdown-link--all"><span>${allServicesLabel}</span><span aria-hidden="true">→</span></a>`,
    );
  });
}
...
function initNavalParticles() {
  ...
  function r() {
    for (t.clearRect(0, 0, e.width, e.height); n.length < 28; ) n.push(i());
...
function initConversionTracking() {
  if (
    ...
    document.documentElement.hasAttribute("data-cbd-conversion-tracking")
  )
    return;
  ...
  document.addEventListener("click", function (t) {
    ...
    gtag("event", "contact", {
      ...
      event_label:
        method === "whatsapp" && a.classList.contains("whatsapp-float")
          ? "boton_flotante"
          : e,
